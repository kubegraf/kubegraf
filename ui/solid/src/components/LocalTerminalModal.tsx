...
const LocalTerminalModal: Component<LocalTerminalModalProps> = (props) => {
  ...
  createEffect(() => {
    if (props.isOpen) {
      console.log('Modal opened, initializing terminal...');
    ...
    <Modal isOpen={props.isOpen} onClose={handleClose} title="Local Terminal" size="full">
      <div class="flex flex-col" style={{ height: '80vh', minHeight: '600px' }}>
        ...
        <div class="flex items-center gap-2 mb-2 px-3 py-2 rounded-lg" style={{ background: 'var(--bg-tertiary)' }}>
          ...
          <button
            onClick={openTerminalInNewWindow}
            class="ml-2 px-3 py-1.5 rounded text-xs font-medium transition-all hover:opacity-80"
            style={{ 
              background: 'var(--accent-primary)', 
              color: 'white',
            }}
            title="Open in new window (recommended)"
          >
            Open in New Window
          </button>
        </div>
