...
interface CustomAppDeleteModalProps {
  isOpen: boolean;
  appName: string;
  displayName: string;
  onClose: () => void;
...
const CustomAppDeleteModal: Component<CustomAppDeleteModalProps> = (props) => {
  return (
    <Modal
      isOpen={props.isOpen}
    ...
    >
      <div class="space-y-4">
        <div class="flex items-start gap-3">
          <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'rgba(245, 158, 11, 0.15)' }}>
          ...
          <div class="flex-1">
            <p class="text-sm mb-3" style={{ color: 'var(--text-primary)' }}>
              Are you sure you want to remove <strong>{props.displayName}</strong> from your custom apps?
            </p>
            <div class="p-3 rounded-lg mb-4" style={{ background: 'var(--bg-tertiary)' }}>
              ...
              <div class="text-xs" style={{ color: 'var(--text-muted)' }}>
                App ID: {props.appName}
              </div>
            </div>
            <p class="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
              This will remove the app from your custom apps list. It will not uninstall any installed instances.
            </p>
          </div>
        </div>
        <div class="flex justify-end gap-3 pt-4 border-t" style={{ 'border-color': 'var(--border-color)' }}>
          ...
          >
            <Show when={props.loading} fallback="Remove">
