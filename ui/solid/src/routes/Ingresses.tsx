  ...
  return (
    <div class="space-y-6">
      ...
      <div class="card overflow-hidden">
        <Show when={!ingresses.loading} fallback={<div class="p-8 text-center"><div class="spinner mx-auto" /></div>}>
          <table class="data-table">
            <thead><tr><th>Name</th><th>Namespace</th><th>Class</th><th>Hosts</th><th>Address</th><th>Ports</th><th>Age</th><th>Actions</th></tr></thead>
            <tbody>
              <For each={filtered()} fallback={<tr><td colspan="8" class="text-center py-8" style={{ color: 'var(--text-muted)' }}>No Ingresses found</td></tr>}>
                {(ing: Ingress) => (
                  <tr>
                    <td class="font-medium" style={{ color: 'var(--accent-primary)' }}>{ing.name}</td>
                    <td>{ing.namespace}</td>
                    <td>{ing.class || '-'}</td>
                    <td>
                      <div class="flex flex-wrap gap-1">
                        <For each={ing.hosts || []}>
                          {(host) => (
                            <a href={`https://${host}`} target="_blank" class="text-xs px-2 py-1 rounded"
                    ...
                    <td>{ing.age}</td>
                    <td>
                      <button onClick={() => { setSelected(ing); setShowYaml(true); }} class="action-btn" title="YAML">
