...
const Jobs: Component = () => {
  ...
  const filtered = createMemo(() => {
    ...
    if (!query) return all;
    return all.filter((j: Job) => j.name.toLowerCase().includes(query) || j.namespace.toLowerCase().includes(query));
  });
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="card overflow-hidden">
        <Show when={!jobs.loading} fallback={<div class="p-8 text-center"><div class="spinner mx-auto" /></div>}>
          <table class="data-table">
            <thead><tr><th>Name</th><th>Namespace</th><th>Completions</th><th>Duration</th><th>Age</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              <For each={filtered()} fallback={<tr><td colspan="7" class="text-center py-8" style={{ color: 'var(--text-muted)' }}>No Jobs found</td></tr>}>
                {(job: Job) => (
                  <tr>
                    <td class="font-medium" style={{ color: 'var(--accent-primary)' }}>{job.name}</td>
                    <td>{job.namespace}</td>
                    <td>{job.completions}</td>
                    <td>{job.duration || '-'}</td>
                    <td>{job.age}</td>
                    <td><span class={`badge ${job.status === 'Complete' ? 'badge-success' : job.status === 'Failed' ? 'badge-error' : 'badge-warning'}`}>{job.status}</span></td>
                    <td>
                      <button onClick={() => { setSelected(job); setShowYaml(true); }} class="action-btn" title="YAML">
