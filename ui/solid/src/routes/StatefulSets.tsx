...
const StatefulSets: Component = () => {
  ...
  const deleteStatefulSet = async (sts: StatefulSet) => {
    if (!confirm(`Are you sure you want to delete StatefulSet ${sts.name}?`)) return;
    try {
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between flex-wrap gap-4">
        ...
        <div class="flex items-center gap-3">
          <button onClick={() => refetch()} class="p-2 rounded-lg hover:bg-[var(--bg-tertiary)]" style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)' }} title="Refresh StatefulSets">
